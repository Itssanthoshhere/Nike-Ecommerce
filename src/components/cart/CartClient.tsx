                    ...
                    {items.map((it) => (
                        <div key={it.cartItemId} className="flex items-start gap-6">
                            <div className="h-28 w-40 flex-shrink-0 overflow-hidden rounded-md bg-light-200">
                                {it.imageUrl && (
                                    <Image src={it.imageUrl} alt={it.title} width={160} height={112}
                            ...
                            <div className="flex-1">
                                ...
                                <div className="mt-2 flex items-center gap-6 text-sm">
                                    ...
                                    <div className="inline-flex items-center rounded-full border border-light-300">
                                        <button onClick={() => handleQty(it.cartItemId, Math.max(1, it.quantity - 1))}
                                                className="px-3 py-1">−
                                        </button>
                                        <span className="px-3 py-1">{it.quantity}</span>
                                        <button onClick={() => handleQty(it.cartItemId, it.quantity + 1)}
                                                className="px-3 py-1">+
                                        </button>
                                    </div>
                                </div>
                            ...
                            <div className="flex flex-col items-end gap-3">
                                ...
                                <button aria-label="Remove" onClick={() => handleRemove(it.cartItemId)}
